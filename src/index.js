...
async function placeOrder(coin) {
  ...
  const buyResponse = await api.marketBuy(pair, quantity, (quoteOrderQty > 10 ? quoteOrderQty + 10.02 : quoteOrderQty));
  const sellResponse = buyResponse && quoteOrderQty > 10  ? await api.marketSell(pair, quantity, 10.01) : undefined

  const response = {
    ...buyResponse,
    executedQty: quoteOrderQty,
    cummulativeQuoteQty: buyResponse.cummulativeQuoteQty - sellResponse.cummulativeQuoteQty,
  }

  if (response.orderId) {
    const successText = `Successfully purchased: ${response.executedQty}${currency} worth of ${asset} @ ${response.fills[0].price}. Spend: ${response.cummulativeQuoteQty}.\n`;
...
async function runBot() {
  console.log(colors.magenta("Starting DCA Bot on Binance"), colors.grey(`[${new Date().toLocaleString()}]`));
